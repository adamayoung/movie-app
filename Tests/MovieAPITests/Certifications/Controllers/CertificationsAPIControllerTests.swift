//
//  CertificationsAPIControllerTests.swift
//  Movie API
//
...
final class CertificationsAPIControllerTests: XCTestCase {
    ...
    func testShowTVSeries() throws {
        ...
        fetchTVSeriesCertificationsUseCase.result = .success(certifications)

        try app.test(.GET, "certifications/tvseries", afterResponse: { res in
            XCTAssertEqual(res.status, .ok)
        })
    }
